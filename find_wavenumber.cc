...
// -------------------------------------------------------------------------------------
  ...
  else
  {
    ...
    // Solution doc file
    std::string doc_solution_filename = "RESLT_test/critical_soln.dat";
    std::string doc_critical_pressure_filename = "RESLT_test/critical_pressure.dat";
    ...
    unsigned minimum_wavemode_guess = 0;
    unsigned maximum_wavemode_guess = 10;
    ...
    // Flag to say if the eigenvalue increases when the wavemode
    bool eigenvalue_is_increasing = false;

    // First, check that the smallest eigenvalue when using the minimum and
    // maximum wavemode have opposite signs and therefore there may be a root
    // to find within the range provided
    GlobalParameters::Wavemode = minimum_wavemode_guess;
    min_guess_eigenvalue = problem.solve_base_problem_and_find_eigenvalue();

    GlobalParameters::Wavemode = maximum_wavemode_guess;
    ...
    if (max_guess_eigenvalue * min_guess_eigenvalue > 0.0)
    {
      ...
      oomph_info << "wavemode = "<< minimum_wavemode_guess <<" eigen_ = " << min_guess_eigenvalue << std::endl;

      oomph_info << "wavemode = "<< maximum_wavemode_guess <<" eigen_ = " << max_guess_eigenvalue << std::endl;
    ...
    GlobalParameters::Wavemode = minimum_wavemode_guess;
    current_guess_eigenvalue = min_guess_eigenvalue;
          oomph_info << " eigen_0 = " << current_guess_eigenvalue << std::endl;
          oomph_info << " wavemode_0 = " << GlobalParameters::Wavemode << std::endl;


    while (current_guess_eigenvalue < 0)
    {
      // Choose the average pressure as a guess
      GlobalParameters::Wavemode += 1;

      // Compute the smallest eigenvalue
      current_guess_eigenvalue =
      problem.solve_base_problem_and_find_eigenvalue();

      oomph_info << " eigen_it = " << current_guess_eigenvalue << std::endl;
      oomph_info << " wavemode_it = " << GlobalParameters::Wavemode << std::endl;
 

    }
    
    // First unstable wavemode
    oomph_info << " Wavemode = " << GlobalParameters::Wavemode << std::endl;
