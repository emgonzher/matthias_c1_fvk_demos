...
// ===========
  ...
  // {
    ...
    void get_pressure(const Vector<double>& x, double& pressure)
    {
      ...
      //double p_cos = 0.1;
      double theta = 0; // miraqui - define for different cuadrants
      if (x[0]*x[1]>0)
      {
        if (x[1]>0) // 1 cuadrant
        {
        ...
        else if (x[1]<0) //3 cuadrant
        {
          theta = atan(x[1]/x[0]) + MathematicalConstants::Pi;
        }
      }
      else if (x[0]*x[1]<0)
      {
        if (x[1]<0) //4 cuadrant
        {
          theta = atan(x[1]/x[0]) + 2.0*MathematicalConstants::Pi;
        }
        else if (x[1]>0) //2 cuadrant
        {
          theta = -1.0*atan(x[1]/x[0]) + 0.5*MathematicalConstants::Pi;
        }
      }
      oomph_info << theta << std::endl;
      double r2 = x[0]*x[0]+x[1]*x[1];
      pressure = P_mag + P_cos*r2*cos(N_mode*theta);
    }
 // }
// ================
...
// }
 ...
 //return 0;
  ...
  // 2-Loop to increment P_mag
   while (Parameters::P_mag < 1.0 ) // for (unsigned i = 0; i < 35; i++ )
  {
  // Bump
  Parameters::P_mag += 0.100;
